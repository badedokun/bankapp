...
exports.SmartSuggestionsEngine = void 0;
const DatabaseUserDataProvider_1 = require("../data/DatabaseUserDataProvider");
const database_1 = require("../../../config/database");
class SmartSuggestionsEngine {
    constructor(tenantId) {
        this.dataProvider = new DatabaseUserDataProvider_1.DatabaseUserDataProvider(tenantId);
    }
    ...
    async getUserDataFromDatabase(userId) {
        try {
            // Get user balance from tenant-specific database
            const balanceResult = await (0, database_1.query)('SELECT balance, currency FROM tenant.wallets WHERE user_id = $1', [userId]);
    ...
    async getDailySummary(userId) {
        try {
            const result = await (0, database_1.query)(`SELECT COUNT(*) as count, COALESCE(SUM(CASE WHEN type = 'credit' THEN amount ELSE 0 END), 0) as income,
         COALESCE(SUM(CASE WHEN type = 'debit' THEN ABS(amount) ELSE 0 END), 0) as expenses
         FROM tenant.transactions
         WHERE user_id = $1 AND DATE(created_at) = CURRENT_DATE`, [userId]);
            return {
    ...
    detectRecurringPayments(transactions) {
        ...
        return Object.values(amountCounts).filter((count) => count > 1).length;
    }
    ...
    static async getPersonalizedSuggestions(request) {
