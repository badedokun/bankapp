...
exports.fraudDetectionService = exports.AIFraudDetectionService = void 0;
/**
...
class NigerianFraudPatternModel {
    ...
    forwardLayer(inputs, weights, biases, isOutput) {
        const outputs = [];
    ...
    calculateVelocityRisk(behavioralData) {
    ...
    calculateAmountAnomaly(currentAmount, avgAmount) {
        if (avgAmount === 0)
    ...
    calculateTypingAnomalyScore(pattern) {
        if (pattern.length === 0)
            return 0.5;
    ...
    calculateMouseMovementAnomalyScore(movements) {
    ...
    calculateAmountDistributionScore(amount) {
...
class NetworkSecurityAnalyzer {
    async analyzeNetwork(ipAddress, userAgent) {
    ...
    async detectVPN(ipAddress, userAgent) {
        ...
        const hasVPNInUserAgent = vpnIndicators.some(indicator => userAgent.toLowerCase().includes(indicator));
    ...
    calculateNetworkRiskScore(analysis) {
        let score = 0;
        if (analysis.isVPN)
            score += 30;
        if (analysis.isProxy)
        ...
        if (analysis.isTor)
            score += 40;
        if (analysis.threatIntelligence.isKnownThreat)
            score += 50;
    ...
    isKnownBotnet(ipAddress) {
        // Check against botnet IP databases
        return false; // Safe default
    }
    isKnownMalware(ipAddress) {
        // Check against malware command & control servers
        return false; // Safe default
    }
}
NetworkSecurityAnalyzer.VPN_PROVIDERS = [
    'NordVPN', 'ExpressVPN', 'Surfshark', 'CyberGhost', 'Private Internet Access'
];
NetworkSecurityAnalyzer.KNOWN_PROXY_PORTS = [3128, 8080, 8888, 1080, 9050];
/**
...
class AIFraudDetectionService {
    ...
    calculateBehavioralRiskScore(request) {
        let score = 0;
        const { behavioralData } = request;
        // Session duration anomalies
        if (behavioralData.sessionDuration < 30)
            score += 0.3; // Too quick
        if (behavioralData.sessionDuration > 3600)
            score += 0.2; // Too long
    ...
    calculateTransactionRiskScore(request) {
        let score = 0;
        ...
        if (transactionData.amount % 100000 === 0)
            score += 0.2;
    ...
    generateFlags(mlScore, networkAnalysis, request) {
        const flags = [];
        // ML-based flags
        if (mlScore > 0.7)
            flags.push('ml_high_risk');
        if (mlScore > 0.9)
            flags.push('ml_critical_risk');
        // Network-based flags
        if (networkAnalysis.isVPN)
            flags.push('vpn_detected');
        if (networkAnalysis.isProxy)
            flags.push('proxy_detected');
        if (networkAnalysis.isTor)
            flags.push('tor_detected');
        if (networkAnalysis.threatIntelligence.isKnownThreat)
            flags.push('threat_ip');
        // Behavioral flags
        const { behavioralData } = request;
        if (behavioralData.sessionDuration < 30)
        ...
        // Transaction flags
        if (request.transactionData.amount > 1000000)
            flags.push('high_value');
        if (request.transactionData.amount % 100000 === 0)
    ...
    generateRecommendations(flags, riskLevel) {
    ...
    calculateConfidence(mlScore, networkAnalysis) {
...
exports.AIFraudDetectionService = AIFraudDetectionService;
// Export singleton instance
exports.fraudDetectionService = new AIFraudDetectionService();
