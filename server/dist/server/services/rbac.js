...
class RBACService {
     ...
     */
    async getUserPermissionLevel(tenantId, userId, permissionCode) {
        const query = `
     ...
     */
    async checkUserPermission(tenantId, userId, permissionCode, resourceId) {
        const query = `
     ...
     */
    async getUserAvailableFeatures(tenantId, userId) {
        ...
        const isFullAccess = legacyRole === 'admin' || userRoles.some(role => ['ceo', 'platform_admin', 'tenant_admin'].includes(role.roleCode));
     ...
     */
    async logPermissionCheck(tenantId, userId, resource, action, permissionCode, accessGranted, denialReason, requestContext) {
        const query = `
     ...
     * Assign role to user
     */
    async assignUserRole(tenantId, userId, roleCode, assignedBy, reason, effectiveFrom, effectiveTo) {
        const query = `
     ...
     */
    async removeUserRole(tenantId, userId, roleCode) {
        const query = `
     ...
     */
    async getRoleBasedMetrics(tenantId, userId) {
        const userRoles = await this.getUserRoles(tenantId, userId);
        const permissions = await this.getUserPermissions(tenantId, userId);
        const isManager = userRoles.some(role => ['ceo', 'coo', 'cfo', 'branch_manager', 'operations_manager'].includes(role.roleCode));
