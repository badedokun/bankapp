...
exports.CanadaComplianceProvider = void 0;
const IComplianceProvider_1 = require("./IComplianceProvider");
/**
...
class CanadaComplianceProvider extends IComplianceProvider_1.BaseComplianceProvider {
    constructor() {
        super(...arguments);
        this.name = 'Canada Compliance Provider';
        this.region = 'Canada';
        this.country = 'CA';
        this.capabilities = {
            supportsKYC: true,
    ...
    async performKYC(request) {
        ...
        if (level === 'basic') {
            ...
            const photoId = documents.find(d => d.type === 'drivers_license' || d.type === 'passport');
            if (photoId) {
     ...
     */
    async monitorTransactions(request) {
        this.ensureInitialized();
        ...
        const highRiskTransactions = transactions.filter(t => t.amount >= 10000 || (t.recipientCountry && HIGH_RISK_COUNTRIES.includes(t.recipientCountry))).length;
        // Velocity checks
        if (checkVelocity) {
            const last24Hours = transactions.filter(t => t.timestamp >= new Date(Date.now() - 24 * 60 * 60 * 1000));
        ...
        if (checkThresholds) {
            const largeCash = transactions.filter(t => t.type === 'deposit' && t.amount >= 10000);
            ...
            const internationalEFT = transactions.filter(t => t.amount >= 10000 && t.recipientCountry && t.recipientCountry !== 'CA');
     ...
     */
    async getRegulatoryLimits(currency) {
        this.ensureInitialized();
    ...
    // Helper methods
    async verifyPrivacyConsent(userId) {
    ...
    async performEnhancedCreditCheck(user) {
    ...
    async screenAdverseMedia(user) {
        // Mock adverse media screening
        return true; // No adverse media
    }
    async verifyBiometrics(user) {
        // Mock biometric verification
        return true;
    }
    async checkThirdPartyDetermination(user) {
        // PCMLTFA: Determine if conducting transaction on behalf of third party
        return { valid: true };
    }
    calculateRiskLevel(score, checks, user) {
        if (!checks.sanctionsCheck || user.politicallyExposed)
            return 'high';
        if (score >= 80)
            return 'low';
        if (score >= 60)
    ...
    getRecentTransactions(history, hours) {
        const cutoff = new Date();
        cutoff.setHours(cutoff.getHours() - hours);
        return history.filter(t => t.timestamp >= cutoff);
    }
    detectStructuring(transactions, threshold) {
        const nearThreshold = transactions.filter(t => t.amount >= threshold * 0.75 && t.amount < threshold);
    ...
    calculateUserBaseline(history) {
        if (history.length === 0)
            return { averageAmount: 0, typicalTypes: new Set() };
    ...
    getRecommendation(score, flags) {
        if (flags.sanctionsMatch || flags.structuring)
            return 'reject';
        if (score >= 60)
            return 'escalate';
        if (score >= 40)
    ...
    getSanctionsRiskLevel(score, matches) {
        if (score >= 95)
            return 'critical';
        if (score >= 85)
            return 'high';
    ...
    detectRoundRobinPattern(transactions) {
    ...
    detectGeographicAnomalies(transactions) {
        const highRiskTxs = transactions.filter(t => t.recipientCountry && HIGH_RISK_COUNTRIES.includes(t.recipientCountry));
    ...
    detectSmurfing(transactions) {
        ...
        const smallDeposits = transactions.filter(t => t.type === 'deposit' && t.amount >= 1000 && t.amount < 5000);
...
exports.CanadaComplianceProvider = CanadaComplianceProvider;
//# sourceMappingURL=CanadaComplianceProvider.js.map