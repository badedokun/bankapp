...
const USAComplianceProvider_1 = require("./USAComplianceProvider");
const EuropeComplianceProvider_1 = require("./EuropeComplianceProvider");
const CanadaComplianceProvider_1 = require("./CanadaComplianceProvider");
class ComplianceService {
    constructor(db) {
        this.initialized = false;
        this.db = db;
        this.registry = IComplianceProvider_1.complianceProviderRegistry;
    }
    /**
    ...
    async initialize() {
        ...
        const usaProvider = new USAComplianceProvider_1.USAComplianceProvider();
        ...
        const europeProvider = new EuropeComplianceProvider_1.EuropeComplianceProvider();
        ...
        const canadaProvider = new CanadaComplianceProvider_1.CanadaComplianceProvider();
     ...
     */
    async performKYC(request, tenantId) {
     ...
     */
    async checkAML(request, tenantId) {
        const provider = await this.selectProvider({
            tenantId,
            country: request.user.countryOfResidence
        });
     ...
     */
    async checkSanctions(request, tenantId) {
        const provider = await this.selectProvider({
            tenantId,
     ...
     */
    async monitorTransactions(request, tenantId) {
     ...
     */
    async generateReport(request, tenantId) {
        const provider = await this.selectProvider({ tenantId });
        if (!provider) {
            throw new Error('No compliance provider available for this request');
        }
     ...
     */
    async submitReport(report, tenantId) {
        const provider = await this.selectProvider({ tenantId });
        if (!provider) {
            throw new Error('No compliance provider available for this request');
        }
     ...
     */
    async getRegulatoryLimits(currency, tenantId) {
        const provider = await this.selectProvider({ tenantId });
        if (!provider) {
            throw new Error('No compliance provider available for this request');
        }
     ...
     */
    async getRequiredKYCLevel(operation, amount, currency, tenantId) {
        const provider = await this.selectProvider({ tenantId });
        if (!provider) {
     ...
     */
    async isOperationCompliant(operation, context, tenantId) {
        const provider = await this.selectProvider({ tenantId });
        if (!provider) {
            return false;
        }
    ...
    async getAvailableProviders(tenantId) {
        ...
        return allProviders.filter(provider => provider.country === tenantConfig.country ||
     ...
     */
    async performComprehensiveCheck(user, transaction, tenantId) {
        const provider = await this.selectProvider({
            tenantId,
        ...
        const maxRisk = Math.max(riskScores[kyc.riskLevel], riskScores[aml.riskLevel], riskScores[sanctions.riskLevel]);
        const overallRisk = Object.keys(riskScores).find(key => riskScores[key] === maxRisk);
        // Determine approval
        const approved = kyc.verified &&
            aml.passed &&
            !sanctions.matched &&
...
function getComplianceService(db) {
