...
exports.EuropeComplianceProvider = void 0;
...
class EuropeComplianceProvider extends IComplianceProvider_1.BaseComplianceProvider {
    constructor() {
        super(...arguments);
        this.name = 'Europe Compliance Provider';
        this.region = 'Europe';
        this.country = 'EU';
        this.capabilities = {
            supportsKYC: true,
    ...
    async performKYC(request) {
        ...
        if (level === 'basic') {
            ...
            const nationalId = documents.find(d => d.type === 'national_id' || d.type === 'passport');
            if (nationalId) {
    ...
    async monitorTransactions(request) {
        ...
        if (checkThresholds) {
            const crossBorderLarge = transactions.filter(t => t.amount >= 10000 && t.recipientCountry && !EU_MEMBER_STATES.includes(t.recipientCountry));
    ...
    // Helper methods
    async verifyGDPRConsent(userId) {
    ...
    async verifyAddressOfficial(address) {
        // Verify address through official records
    ...
    async verifyBeneficialOwnership(user) {
    ...
    getRecentTransactions(history, days) {
        const cutoff = new Date();
        cutoff.setDate(cutoff.getDate() - days);
        return history.filter(t => t.timestamp >= cutoff);
    }
    detectStructuring(transactions, threshold) {
        const nearThreshold = transactions.filter(t => t.amount >= threshold * 0.8 && t.amount < threshold);
    ...
    async checkSCAREquirement(transaction) {
        // PSD2 SCA exemptions
        if (transaction.amount <= 30)
            return false; // Low-value exemption
    ...
    detectThirdPartyPayments(transactions) {
...
exports.EuropeComplianceProvider = EuropeComplianceProvider;
//# sourceMappingURL=EuropeComplianceProvider.js.map