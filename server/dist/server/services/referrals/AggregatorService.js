...
class AggregatorService {
    ...
    async createPartner(params) {
        ...
        try {
            const result = await pool.query(`INSERT INTO aggregators.partners (
    ...
    async getPartnerById(partnerId) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.partners WHERE id = $1`, [partnerId]);
    ...
    async getPartnerByCode(code) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.partners WHERE custom_code = $1`, [code.toUpperCase()]);
            return result.rows[0] || null;
        }
        catch (error) {
    ...
    async getPartners(filters) {
        ...
        try {
            ...
            const result = await pool.query(`SELECT * FROM aggregators.partners
         ${whereClause}
     ...
     */
    async updatePartnerStatus(partnerId, status, reason) {
        const pool = await this.getPool();
        try {
            await pool.query(`UPDATE aggregators.partners
         SET status = $1, status_reason = $2, updated_at = CURRENT_TIMESTAMP
         WHERE id = $3`, [status, reason || null, partnerId]);
        }
        catch (error) {
     ...
     */
    async calculateCompensation(partnerId, periodStart, periodEnd) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.calculate_compensation($1, $2, $3)`, [partnerId, periodStart, periodEnd]);
     ...
     */
    async generateMonthlyPayouts(year, month) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT aggregators.generate_monthly_payouts($1, $2) as generate_monthly_payouts`, [year, month]);
     ...
     * Get partner payouts
     */
    async getPartnerPayouts(partnerId, limit = 50, offset = 0) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.payouts
         WHERE partner_id = $1
    ...
    async getPayoutById(payoutId) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.payouts WHERE id = $1`, [payoutId]);
            return result.rows[0] || null;
        }
        catch (error) {
     ...
     * Approve payout
     */
    async approvePayout(payoutId, approvedBy, paymentReference) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT aggregators.approve_payout($1, $2, $3) as approve_payout`, [payoutId, approvedBy, paymentReference || null]);
    ...
    async rejectPayout(payoutId, rejectedBy, reason) {
        const pool = await this.getPool();
        try {
            await pool.query(`UPDATE aggregators.payouts
         SET status = 'rejected',
             rejected_by = $1,
             rejected_at = CURRENT_TIMESTAMP,
             rejection_reason = $2,
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $3`, [rejectedBy, reason, payoutId]);
        }
        catch (error) {
     ...
     * Mark payout as paid
     */
    async markPayoutPaid(payoutId, paymentDate, paymentReference) {
        const pool = await this.getPool();
        try {
            await pool.query(`UPDATE aggregators.payouts
         SET status = 'paid',
             ...
             payment_reference = $2,
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $3`, [paymentDate, paymentReference, payoutId]);
        }
        catch (error) {
    ...
    async getPartnerStats(partnerId) {
        ...
        try {
            ...
            // Get current tier
            const tierResult = await pool.query(`SELECT t.tier_name
         FROM aggregators.compensation_tiers t
         WHERE t.id = $1`, [partner.compensationTierId]);
            ...
            const thisMonthResult = await pool.query(`SELECT COUNT(*) as count
         FROM tenant.referrals
         WHERE referrer_id IN (
           SELECT id FROM tenant.users WHERE referral_code = $1
         )
         AND created_at >= DATE_TRUNC('month', CURRENT_DATE)`, [partner.customCode]);
            const thisMonthReferrals = parseInt(thisMonthResult.rows[0].count);
            // Get last month's referrals
            const lastMonthResult = await pool.query(`SELECT COUNT(*) as count
         FROM tenant.referrals
         WHERE referrer_id IN (
           SELECT id FROM tenant.users WHERE referral_code = $1
         )
         ...
         AND created_at < DATE_TRUNC('month', CURRENT_DATE)`, [partner.customCode]);
            ...
            const monthlyResult = await pool.query(`SELECT TO_CHAR(created_at, 'YYYY-MM') as month, COUNT(*) as count
         FROM tenant.referrals
         WHERE referrer_id IN (
           SELECT id FROM tenant.users WHERE referral_code = $1
         )
         ...
         ORDER BY month DESC`, [partner.customCode]);
            return {
     ...
     */
    async getCompensationTiers() {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.compensation_tiers ORDER BY tier_level ASC`);
            return result.rows;
        }
        catch (error) {
     ...
     */
    async getPayoutsByStatus(status, limit = 50, offset = 0) {
        const pool = await this.getPool();
        try {
            const result = await pool.query(`SELECT * FROM aggregators.payouts
         WHERE status = $1
