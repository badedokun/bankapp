...
exports.ConversationalTransferService = void 0;
const multi_tenant_database_1 = __importDefault(require("../../config/multi-tenant-database"));
const ConversationStateManager_1 = __importDefault(require("./ConversationStateManager"));
const bcrypt_1 = __importDefault(require("bcrypt"));
const referenceGenerator_1 = require("../../utils/referenceGenerator");
class ConversationalTransferService {
    /**
     * Process message in conversational transfer flow
     */
    static async processTransferConversation(message, userId, conversationId, tenantId) {
        try {
            const state = ConversationStateManager_1.default.getState(userId, conversationId);
            // Check if starting new transfer
            if (!state.currentFlow || state.currentFlow !== 'transfer') {
                // Initialize transfer flow
                ConversationStateManager_1.default.updateState(userId, conversationId, {
            ...
            if (message.toLowerCase().includes('cancel')) {
                ConversationStateManager_1.default.clearState(userId, conversationId);
                return {
            ...
            if (currentStep.field === 'amount') {
                ...
                const bank = this.NIGERIAN_BANKS.find(b => b.name.toLowerCase().includes(message.toLowerCase()) || b.code === message);
                processedValue = bank?.code;
                ConversationStateManager_1.default.setData(userId, conversationId, 'bankName', bank?.name);
            }
            else if (currentStep.field === 'description' && !message.trim()) {
                processedValue = 'Transfer from OrokiiPay';
            }
            ConversationStateManager_1.default.setData(userId, conversationId, currentStep.field, processedValue);
            ...
            // Move to next step
            ConversationStateManager_1.default.nextStep(userId, conversationId);
            const updatedState = ConversationStateManager_1.default.getState(userId, conversationId);
            ...
            if (nextStep.field === 'accountName' && nextStep.process) {
                const accountNumber = ConversationStateManager_1.default.getData(userId, conversationId, 'accountNumber');
                const result = await nextStep.process(accountNumber, userId);
                ConversationStateManager_1.default.setData(userId, conversationId, 'accountName', result.accountName);
                ConversationStateManager_1.default.nextStep(userId, conversationId);
                const afterVerificationState = ConversationStateManager_1.default.getState(userId, conversationId);
                ...
                const confirmationMessage = `✅ Account verified!\n\n` +
                    ...
                    `Bank: ${ConversationStateManager_1.default.getData(userId, conversationId, 'bankName')}\n` +
                    `Amount: ₦${ConversationStateManager_1.default.getData(userId, conversationId, 'amount').toLocaleString()}\n\n` +
     ...
     */
    static async executeTransfer(userId, conversationId, data, tenantId) {
        try {
     ...
     */
    static async verifyPIN(userId, pin, tenantId) {
        try {
            const result = await multi_tenant_database_1.default.queryTenant(tenantId, 'SELECT transaction_pin_hash FROM tenant.users WHERE id = $1', [userId]);
            if (result.rows.length === 0 || !result.rows[0].transaction_pin_hash) {
                return false;
            }
            const pinHash = result.rows[0].transaction_pin_hash;
            const isValid = await bcrypt_1.default.compare(pin, pinHash);
            return isValid;
        }
        catch (error) {
     ...
     */
    static async checkBalance(userId, amount, tenantId) {
        try {
            // Try to get wallet with any common wallet type
            const result = await multi_tenant_database_1.default.queryTenant(tenantId, `SELECT balance, wallet_type FROM tenant.wallets
         WHERE user_id = $1
     ...
     * Create transfer record
     */
    static async createTransfer(userId, data, tenantId) {
        // Get a database client for transaction
        const client = await multi_tenant_database_1.default.getTenantClient(tenantId);
        try {
            // Start transaction
            await client.query('BEGIN');
            // Get tenant's bank code for reference generation
            const tenantResult = await multi_tenant_database_1.default.queryPlatform('SELECT bank_code FROM platform.tenants WHERE id = $1', [tenantId]);
            const bankCode = tenantResult.rows[0]?.bank_code || 'ORP';
            // Generate secure ULID-based reference
            const reference = (0, referenceGenerator_1.generateTransferRef)(bankCode);
            // Get wallet FIRST to ensure it exists and has sufficient balance
            const walletResult = await client.query(`SELECT id, wallet_type, balance, wallet_number FROM tenant.wallets
         WHERE user_id = $1
         AND wallet_type IN ('primary', 'main', 'default', 'checking')
         ORDER BY
           CASE wallet_type
         ...
         LIMIT 1`, [userId]);
            ...
            const updateResult = await client.query(`UPDATE tenant.wallets
         SET balance = balance - $1, updated_at = NOW()
         WHERE id = $2
         RETURNING balance`, [data.amount, wallet.id]);
            ...
            const transferResult = await client.query(`INSERT INTO tenant.transfers (
...
exports.ConversationalTransferService = ConversationalTransferService;
_a = ConversationalTransferService;
ConversationalTransferService.NIGERIAN_BANKS = [
...
ConversationalTransferService.TRANSFER_STEPS = [
    {
        step: 1,
        field: 'amount',
    ...
    },
    {
        ...
        validation: (value) => {
            const bank = _a.NIGERIAN_BANKS.find(b => b.name.toLowerCase().includes(value.toLowerCase()) || b.code === value);
            if (!bank) {
    ...
    },
    {
        ...
        process: async (accountNumber, userId) => {
...
exports.default = ConversationalTransferService;
//# sourceMappingURL=ConversationalTransferService.js.map