...
class RewardService {
    ...
    async getUserRewards(userId) {
      ...
      WHERE ur.user_id = $1`, [userId]);
        ...
        // Get next tier
        const nextTierResult = await pool.query(`SELECT * FROM rewards.tiers
       WHERE tier_level > $1
       ORDER BY tier_level ASC
       LIMIT 1`, [row.tier_level]);
        const currentTier = {
            id: row.tier_id,
    ...
    async initializeUserRewards(userId) {
        ...
        const tierResult = await pool.query(`SELECT id FROM rewards.tiers WHERE tier_code = 'bronze' LIMIT 1`);
        const bronzeTierId = tierResult.rows[0]?.id;
        await pool.query(`INSERT INTO rewards.user_rewards (user_id, total_points, current_tier_id, points_this_month, lifetime_points)
       VALUES ($1, 0, $2, 0, 0)
       ON CONFLICT (user_id) DO NOTHING`, [userId, bronzeTierId]);
        // Award welcome bonus
     ...
     */
    async awardPoints(userId, points, actionType, description, metadata) {
        const pool = await this.getPool();
        await pool.query(`SELECT rewards.award_points($1, $2, $3, $4, $5)`, [userId, points, actionType, description, metadata ? JSON.stringify(metadata) : null]);
    ...
    async checkTierUpgrade(userId) {
      ...
      WHERE ur.user_id = $1`, [userId]);
        ...
        if (row.next_tier_id && row.total_points >= row.next_tier_points) {
            await pool.query(`UPDATE rewards.user_rewards
         SET current_tier_id = $1
         WHERE user_id = $2`, [row.next_tier_id, userId]);
            // Award tier upgrade bonus
            const bonusPoints = Math.floor(row.next_tier_points * 0.1); // 10% bonus
            await this.awardPoints(userId, bonusPoints, 'tier_upgrade', `Congratulations! You've reached ${row.next_tier_name} tier! 🎊`, { newTier: row.next_tier_name });
    ...
    async unlockAchievement(userId, achievementCode) {
        ...
        const achievementResult = await pool.query(`SELECT * FROM rewards.achievements WHERE achievement_code = $1 AND is_active = true`, [achievementCode]);
        if (achievementResult.rows.length === 0)
            return false;
        const achievement = achievementResult.rows[0];
        // Check if already unlocked
        const existingResult = await pool.query(`SELECT id FROM rewards.user_achievements WHERE user_id = $1 AND achievement_id = $2`, [userId, achievement.id]);
        if (existingResult.rows.length > 0)
            return false; // Already unlocked
        // Unlock achievement
        await pool.query(`INSERT INTO rewards.user_achievements (user_id, achievement_id, points_earned)
       VALUES ($1, $2, $3)`, [userId, achievement.id, achievement.points_reward]);
        // Award points
        await this.awardPoints(userId, achievement.points_reward, 'achievement_unlocked', `Achievement unlocked: ${achievement.achievement_name}! 🏆`, { achievementCode });
        return true;
    }
    // ==========================================================================
    // Challenges
     ...
     */
    async updateChallengeProgress(userId, challengeCode, progress) {
        const pool = await this.getPool();
        const challengeResult = await pool.query(`SELECT * FROM rewards.challenges WHERE challenge_code = $1`, [challengeCode]);
        if (challengeResult.rows.length === 0)
            return;
        const challenge = challengeResult.rows[0];
        const completionTarget = challenge.completion_criteria.min_count || challenge.completion_criteria.count || 1;
        await pool.query(`INSERT INTO rewards.user_challenges (user_id, challenge_id, progress, status)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id, challenge_id) DO UPDATE SET
        progress = $3,
        status = CASE WHEN $3 >= $5 THEN 'completed' ELSE 'active' END,
        completed_at = CASE WHEN $3 >= $5 THEN CURRENT_TIMESTAMP ELSE NULL END`, [userId, challenge.id, progress, progress >= completionTarget ? 'completed' : 'active', completionTarget]);
     ...
     */
    async claimChallenge(userId, challengeCode) {
        const pool = await this.getPool();
        const result = await pool.query(`UPDATE rewards.user_challenges uc
       ...
       RETURNING c.points_reward, c.challenge_name`, [userId, challengeCode]);
        if (result.rows.length === 0)
            return false;
        const { points_reward, challenge_name } = result.rows[0];
        // Award points
        await this.awardPoints(userId, points_reward, 'challenge_completed', `Challenge completed: ${challenge_name}! 🎯`, { challengeCode });
        return true;
    }
    // ==========================================================================
    // Streaks
    ...
    async getUserStreaks(userId) {
        const pool = await this.getPool();
        const result = await pool.query(`SELECT * FROM rewards.user_streaks WHERE user_id = $1`, [userId]);
        return result.rows.map(row => ({
            userId: row.user_id,
    ...
    async updateStreak(userId, streakType) {
        ...
        const result = await pool.query(`SELECT * FROM rewards.user_streaks WHERE user_id = $1 AND streak_type = $2`, [userId, streakType]);
        if (result.rows.length === 0) {
            // Create new streak
            await pool.query(`INSERT INTO rewards.user_streaks (user_id, streak_type, current_count, longest_count, last_activity_date)
         VALUES ($1, $2, 1, 1, $3)`, [userId, streakType, today]);
        ...
        if (daysDiff === 0) {
            ...
            await pool.query(`UPDATE rewards.user_streaks
         SET current_count = $1, longest_count = $2, last_activity_date = $3
         WHERE user_id = $4 AND streak_type = $5`, [newCount, newLongest, today, userId, streakType]);
         ...
         WHERE user_id = $2 AND streak_type = $3`, [today, userId, streakType]);
        }
    }
    // ==========================================================================
    // Point Transactions
    ...
    async getPointTransactions(userId, limit = 50) {
        const pool = await this.getPool();
        const result = await pool.query(`SELECT * FROM rewards.point_transactions
       WHERE user_id = $1
