...
router.post('/fraud-check', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    ...
    (0, express_validator_1.body)('recipientBankCode').isLength({ min: 1, max: 10 }).withMessage('Bank code must be 1-10 characters'),
...
router.post('/account-inquiry', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('accountNumber').isLength({ min: 10, max: 10 }).withMessage('Account number must be 10 digits'),
    (0, express_validator_1.body)('bankCode').isLength({ min: 1, max: 10 }).withMessage('Bank code must be 1-10 characters'),
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const validationErrors = (0, express_validator_1.validationResult)(req);
    if (!validationErrors.isEmpty()) {
        return res.status(400).json({
...
router.post('/initiate', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('recipientAccountNumber').isLength({ min: 10, max: 10 }).withMessage('Account number must be 10 digits'),
    (0, express_validator_1.body)('recipientBankCode').notEmpty().withMessage('Bank code is required'),
    ...
    try {
      ...
      `, [userId, tenantId]);
            ...
            const platformResult = await multi_tenant_database_1.default.queryPlatform(`
        SELECT bank_code FROM platform.tenants WHERE id = $1
      `, [tenantId]);
            const sourceBankCode = platformResult.rows[0]?.bank_code || 'EXT';
      ...
      `, [today, currentMonth + '-01', userId]);
            ...
            const reference = (0, referenceGenerator_1.generateTransferRef)(sourceBankCode);
      ...
      `, [
                ...
                wallet.source_account, sourceBankCode, isInternalTransfer ? 'successful' : 'pending'
            ]);
...
router.get('/status/:reference', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const userId = req.user?.id;
    const tenantId = req.user?.tenantId;
    const transferResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
    SELECT t.*,
...
router.get('/history', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const transfers = await multi_tenant_database_1.default.queryTenant(tenantId, `
    SELECT t.*,
           CASE WHEN t.sender_id = $1 THEN 'sent' ELSE 'received' END as direction,
 ...
 */
router.get('/banks', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (_req, res) => {
    try {
...
router.post('/internal', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    ...
    try {
        ...
        const walletQuery = await multi_tenant_database_1.default.queryTenant(tenantId, `
    ...
    `, [req.user?.id, tenantId]);
        ...
        const recipientQuery = await multi_tenant_database_1.default.queryTenant(tenantId, `
 ...
 */
router.get('/:idOrReference', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `, [idOrReference, userId]);
    ...
    const transfer = transferResult.rows[0];
    // Get tenant bank info from platform database (cross-database join not supported)
    const platformResult = await multi_tenant_database_1.default.queryPlatform(`
    SELECT bank_code, display_name FROM platform.tenants WHERE id = $1
  `, [tenantId]);
    const senderBankCode = platformResult.rows[0]?.bank_code || 'EXT';
    const senderBankName = platformResult.rows[0]?.display_name || 'Unknown Bank';
