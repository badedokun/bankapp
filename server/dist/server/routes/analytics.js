...
router.get('/dashboard', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `, [userId, tenantId]);
    ...
    const transactionStatsResult = await (0, database_1.query)(`
    SELECT
      type,
  ...
  `, [userId, tenantId]);
    ...
    const monthlySpendingResult = await (0, database_1.query)(`
    SELECT
  ...
  `, [userId, tenantId]);
    ...
    const categorySpendingResult = await (0, database_1.query)(`
    SELECT
      CASE
