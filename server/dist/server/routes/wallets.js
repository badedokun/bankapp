 ...
 */
router.get('/balance', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const walletResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
    ...
    `, [req.user.id, req.user.tenantId]);
        ...
        const spendingResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
    SELECT
  ...
  `, [today, currentMonth + '-01', req.user.id]);
        ...
        const transfersResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
    SELECT
 ...
 */
router.get('/all', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const tenantId = req.user.tenantId;
    const walletsResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
 ...
 */
router.post('/create', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('walletType').isIn(['savings', 'investment', 'goal']).withMessage('Valid wallet type required'),
    (0, express_validator_1.body)('name').optional().isLength({ min: 3, max: 50 }).withMessage('Wallet name must be 3-50 characters'),
    (0, express_validator_1.body)('description').optional().isLength({ max: 200 }).withMessage('Description too long'),
    (0, express_validator_1.body)('targetAmount').optional().isFloat({ min: 1000 }).withMessage('Target amount must be at least ₦1,000')
    ...
    try {
        ...
        const countResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
    ...
    `, [userId, tenantId, walletType, name, description, targetAmount]);
        ...
        throw errorHandler_1.errors.internalError('Wallet creation failed');
    }
}));
/**
 ...
 */
router.post('/transfer-between', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('fromWalletId').isUUID().withMessage('Valid source wallet ID required'),
    (0, express_validator_1.body)('toWalletId').isUUID().withMessage('Valid destination wallet ID required'),
    ...
    `, [userId]);
        const isPinValid = await bcrypt_1.default.compare(pin, userResult.rows[0].transaction_pin_hash);
        if (!isPinValid) {
 ...
 */
router.post('/fund', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('walletId').optional().isUUID().withMessage('Valid wallet ID required'),
    (0, express_validator_1.body)('amount').isFloat({ min: 100 }).withMessage('Amount must be at least ₦100'),
    (0, express_validator_1.body)('method').isIn(['bank_transfer', 'card', 'ussd']).withMessage('Valid funding method required'),
    (0, express_validator_1.body)('reference').optional().isLength({ min: 5, max: 50 }).withMessage('Reference must be 5-50 characters')
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const validationErrors = (0, express_validator_1.validationResult)(req);
    if (!validationErrors.isEmpty()) {
        return res.status(400).json({
    ...
    `, [userId, tenantId, wallet.id, fundingAmount, method, fundingReference, fundingResult.externalReference]);
        ...
        throw errorHandler_1.errors.internalError('Wallet funding failed');
    }
    finally {
        client.release();
    }
 ...
 */
router.get('/transactions', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
    const tenantId = req.user.tenantId;
    ...
    const transactionsResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
    SELECT
 ...
 */
router.get('/statement', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
    const tenantId = req.user.tenantId;
    ...
    `, [userId, tenantId]);
        ...
        const openingBalanceResult = await multi_tenant_database_1.default.queryTenant(tenantId, `
      SELECT COALESCE(
    ...
    `, [userId, startDate, endDate]);
        ...
        throw errorHandler_1.errors.internalError('Failed to generate statement');
    }
}));
/**
 ...
 */
router.put('/set-pin', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('pin').isLength({ min: 4, max: 4 }).withMessage('PIN must be exactly 4 digits'),
    (0, express_validator_1.body)('confirmPin').isLength({ min: 4, max: 4 }).withMessage('Confirm PIN must be exactly 4 digits'),
    (0, express_validator_1.body)('currentPin').optional().isLength({ min: 4, max: 4 }).withMessage('Current PIN must be exactly 4 digits')
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const validationErrors = (0, express_validator_1.validationResult)(req);
    if (!validationErrors.isEmpty()) {
        return res.status(400).json({
    ...
    `, [userId]);
        ...
        if (user.transaction_pin_hash && currentPin) {
            const isPinValid = await bcrypt_1.default.compare(currentPin, user.transaction_pin_hash);
            if (!isPinValid) {
                return res.status(401).json({
                    success: false,
        ...
        // Hash new PIN
        const saltRounds = 12;
        const pinHash = await bcrypt_1.default.hash(pin, saltRounds);
        // Update PIN
    ...
    `, [pinHash, userId]);
        ...
        throw errorHandler_1.errors.internalError('Failed to set transaction PIN');
    }
}));
/**
 ...
 */
router.post('/verify-pin', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('pin').isLength({ min: 4, max: 4 }).withMessage('PIN must be exactly 4 digits')
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const validationErrors = (0, express_validator_1.validationResult)(req);
    if (!validationErrors.isEmpty()) {
    ...
    `, [userId]);
        ...
        // Verify PIN
        const isPinValid = await bcrypt_1.default.compare(pin, user.transaction_pin_hash);
        if (!isPinValid) {
            return res.status(401).json({
                success: false,
                error: 'Invalid transaction PIN',
        ...
        throw errorHandler_1.errors.internalError('Failed to verify PIN');
    }
}));
/**
 ...
 */
router.get('/limits', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
    const tenantId = req.user.tenantId;
    try {
        // Get user limits
    ...
    `, [today, currentMonth + '-01', userId]);
        ...
        throw errorHandler_1.errors.internalError('Failed to fetch transaction limits');
    }
}));
// Private method for funding simulation
async function simulateFunding(method, amount, reference) {
