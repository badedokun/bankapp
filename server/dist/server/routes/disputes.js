...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const userResult = await multi_tenant_database_1.default.queryTenant(tenant.id, 'SELECT email, phone_number FROM tenant.users WHERE id = $1', [userId]);
        ...
        const disputeNumberResult = await multi_tenant_database_1.default.queryTenant(tenant.id, 'SELECT tenant.generate_dispute_number($1::text, $2::text) as dispute_number', [tenantCode, tenantLocale]);
        const disputeNumber = disputeNumberResult.rows[0].dispute_number;
        // Insert dispute
        const insertResult = await multi_tenant_database_1.default.queryTenant(tenant.id, `INSERT INTO tenant.transaction_disputes (
        dispute_number,
        transaction_id,
      ...
      RETURNING *`, [
        ...
        await multi_tenant_database_1.default.queryTenant(tenant.id, `INSERT INTO tenant.dispute_activity_log (
        dispute_id,
        activity_type,
        performed_by,
        performed_by_name,
...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        const result = await multi_tenant_database_1.default.queryTenant(tenant.id, queryText, params);
        // Get total count
        ...
        const countResult = await multi_tenant_database_1.default.queryTenant(tenant.id, countQueryText, countParams);
...
router.get('/:disputeId', async (req, res) => {
    ...
    try {
        // Get dispute
        const disputeResult = await multi_tenant_database_1.default.queryTenant(tenant.id, `SELECT * FROM tenant.transaction_disputes
       WHERE id = $1 AND user_id = $2`, [disputeId, userId]);
        if (disputeResult.rows.length === 0) {
            return res.status(404).json({ error: 'Dispute not found' });
        }
        const dispute = disputeResult.rows[0];
        // Get activity log
        const activityResult = await multi_tenant_database_1.default.queryTenant(tenant.id, `SELECT * FROM tenant.dispute_activity_log
       WHERE dispute_id = $1
       ORDER BY created_at DESC`, [disputeId]);
        res.json({
            dispute,
 ...
 */
router.patch('/:disputeId/status', async (req, res) => {
    const userId = req.user?.id;
    const userRole = req.user?.role;
    const tenant = req.tenant;
    const { disputeId } = req.params;
    ...
    try {
        ...
        // Update dispute
        const updateResult = await multi_tenant_database_1.default.queryTenant(tenant.id, `UPDATE tenant.transaction_disputes
       SET status = $1,
       ...
       WHERE id = $6
       RETURNING *`, [status, resolutionNotes, resolutionType, refundAmount, userId, disputeId]);
