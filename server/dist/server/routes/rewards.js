...
function createRewardService(req, res) {
    ...
    return new RewardService_1.default(tenantId);
}
// ============================================================================
// Validation Schemas
// ============================================================================
const awardPointsSchema = zod_1.z.object({
    points: zod_1.z.number().int().min(1).max(10000),
    actionType: zod_1.z.string().min(1).max(50),
    description: zod_1.z.string().min(1).max(500),
    metadata: zod_1.z.record(zod_1.z.any()).optional(),
});
const updateChallengeProgressSchema = zod_1.z.object({
    progress: zod_1.z.number().int().min(0),
});
// ============================================================================
// User Rewards Routes
// ============================================================================
...
router.get('/user/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const [userRewards, achievements, challenges, streaks, recentTransactions,] = await Promise.all([
 ...
 */
router.post('/user/:userId/initialize', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        // Authorization check
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
 ...
 */
router.post('/user/:userId/points', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
 ...
 */
router.get('/user/:userId/tier-summary', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        // Authorization check
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
 ...
 */
router.get('/achievements/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        // Authorization check
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
 ...
 */
router.post('/achievements/:userId/unlock/:achievementCode', auth_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.get('/achievements/:userId/preview', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        // Authorization check
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
 ...
 */
router.get('/challenges/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        // Authorization check
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
 ...
 */
router.post('/challenges/:userId/:challengeCode/progress', auth_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.post('/challenges/:userId/:challengeCode/claim', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId, challengeCode } = req.params;
 ...
 */
router.get('/streaks/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        // Authorization check
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
 ...
 */
router.post('/streaks/:userId/:streakType/update', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (!rewardService)
            return;
        await rewardService.updateStreak(userId, streakType);
        // Get updated streaks
        const streaks = await rewardService.getUserStreaks(userId);
 ...
 */
router.get('/transactions/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
