 ...
 */
router.get('/preferences', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
 ...
 */
router.put('/preferences', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
...
router.get('/', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (status !== 'all') {
        filteredNotifications = filteredNotifications.filter(n => status === 'read' ? n.is_read : !n.is_read);
    }
    // Apply pagination
    const paginatedNotifications = filteredNotifications.slice(parseInt(offset.toString()), parseInt(offset.toString()) + parseInt(limit.toString()));
    res.json({
        success: true,
        data: {
 ...
 */
router.get('/settings', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const settings = {
        channels: {
            email: {
                enabled: true,
