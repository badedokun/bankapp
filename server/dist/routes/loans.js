...
async function calculateEligibility(userId, tenantId, loanType) {
    // Get user's transaction history
    const transactionHistory = await (0, database_1.query)(`
    SELECT
  ...
  `, [userId, tenantId]);
    ...
    const repaymentHistory = await (0, database_1.query)(`
    SELECT
 ...
 */
router.post('/personal/apply', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('amount').isFloat({
    ...
    (0, express_validator_1.body)('tenure').isIn(LOAN_CONFIG.personal.tenureOptions)
        .withMessage('Invalid tenure period'),
    (0, express_validator_1.body)('purpose').notEmpty().withMessage('Loan purpose is required'),
    (0, express_validator_1.body)('employmentStatus').isIn(['employed', 'self-employed', 'business-owner'])
 ...
 */
router.post('/business/apply', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('amount').isFloat({
    ...
    (0, express_validator_1.body)('tenure').isIn(LOAN_CONFIG.business.tenureOptions)
        .withMessage('Invalid tenure period'),
    (0, express_validator_1.body)('businessName').notEmpty().withMessage('Business name is required'),
    (0, express_validator_1.body)('businessType').notEmpty().withMessage('Business type is required'),
    (0, express_validator_1.body)('yearsInBusiness').isInt({ min: 1 })
 ...
 */
router.post('/business/documents', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('loanId').notEmpty().withMessage('Loan ID is required'),
    (0, express_validator_1.body)('documentType').isIn(LOAN_CONFIG.business.requiredDocuments)
        .withMessage('Invalid document type'),
    (0, express_validator_1.body)('documentData').notEmpty().withMessage('Document data is required'),
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.get('/business/status/:loanId', auth_1.authenticateToken, tenant_1.validateTenantAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const missingDocuments = LOAN_CONFIG.business.requiredDocuments.filter(doc => { var _a; return !((_a = loanData.uploaded_documents) === null || _a === void 0 ? void 0 : _a.includes(doc)); });
    res.json({
        success: true,
        data: {
            ...loanData,
 ...
 */
router.post('/quick/instant', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('amount').isFloat({ min: 100 })
        .withMessage('Minimum amount is ₦100'),
    (0, express_validator_1.body)('pin').matches(/^\d{4}$/).withMessage('Invalid PIN format'),
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const outstandingLoans = await (0, database_1.query)(`
    ...
    res.json({
        ...
        data: {
            ...
            nextLimit: eligibility.maxAmount < LOAN_CONFIG.quick.maxLimit ?
                LOAN_CONFIG.quick.progressiveLimits[LOAN_CONFIG.quick.progressiveLimits.indexOf(eligibility.maxAmount) + 1] || LOAN_CONFIG.quick.maxLimit : null,
 ...
 * Repay quick loan
 */
router.post('/quick/repay', auth_1.authenticateToken, tenant_1.validateTenantAccess, [
    (0, express_validator_1.body)('loanId').notEmpty().withMessage('Loan ID is required'),
    (0, express_validator_1.body)('amount').optional().isFloat({ min: 100 }),
], (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    await (0, database_1.transaction)(async (client) => {
      ...
      `, [loanId, userId]);
        ...
        const daysOverdue = Math.max(0, Math.floor((Date.now() - new Date(loanData.due_date).getTime()) / (24 * 60 * 60 * 1000)));
        ...
        const wallet = await client.query('SELECT balance FROM tenant.wallets WHERE user_id = $1 AND tenant_id = $2', [userId, tenantId]);
        if (!wallet.rows[0] || wallet.rows[0].balance < repaymentAmount) {
            throw new Error('Insufficient wallet balance');
        }
        // Process repayment
        await client.query('UPDATE tenant.wallets SET balance = balance - $1 WHERE user_id = $2 AND tenant_id = $3', [repaymentAmount, userId, tenantId]);
        await client.query('UPDATE tenant.loans SET status = $1, repaid_at = NOW(), late_fees = $2 WHERE id = $3', ['paid', lateFees, loanId]);
        // Record transaction
        await client.query(`
        INSERT INTO tenant.transactions (
          tenant_id, user_id, type, amount, description, status
