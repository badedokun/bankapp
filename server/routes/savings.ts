 ...
 */
router.post('/flexible/create',
  authenticateToken,
  validateTenantAccess,
  [
    body('accountName').notEmpty().withMessage('Account name is required'),
    body('initialDeposit').isFloat({ min: SAVINGS_CONFIG.flexible.minAmount })
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    await transaction(async (client: any) => {
      ...
      await client.query(
        ...
        [initialDeposit, userId, tenantId]
      );
...
router.post('/flexible/deposit',
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    await transaction(async (client: any) => {
      ...
      const accountResult = await client.query(
        'SELECT * FROM tenant.savings_accounts WHERE id = $1 AND user_id = $2 AND tenant_id = $3',
      ...
      await client.query(
        'UPDATE tenant.savings_accounts SET balance = balance + $1 WHERE id = $2',
        [amount, accountId]
      );
...
router.post('/target/create',
  ...
  [
    ...
    body('targetAmount').isFloat({ min: SAVINGS_CONFIG.target.minAmount })
    ...
    body('initialDeposit').optional().isFloat({ min: 100 }),
  ],
  asyncHandler(async (req: any, res: any) => {
 ...
 */
router.post('/target/contribute',
  authenticateToken,
  validateTenantAccess,
  [
    body('goalId').notEmpty().withMessage('Goal ID is required'),
    body('amount').isFloat({ min: 100 }).withMessage('Minimum contribution is ₦100'),
  ],
  asyncHandler(async (req: any, res: any) => {
    ...
    await transaction(async (client: any) => {
      ...
      const accountResult = await client.query(
        'SELECT * FROM tenant.savings_accounts WHERE id = $1 AND user_id = $2 AND product_type = \'target\'',
        [goalId, userId]
      );
 ...
 */
router.post('/locked/create',
  authenticateToken,
  validateTenantAccess,
  [
    body('accountName').notEmpty().withMessage('Account name is required'),
    body('amount').isFloat({ min: SAVINGS_CONFIG.locked.minAmount })
      .withMessage(`Minimum amount is ₦${SAVINGS_CONFIG.locked.minAmount}`),
    body('lockPeriod').isIn(SAVINGS_CONFIG.locked.lockPeriods)
...
router.post('/group/create',
  ...
  [
    ...
    body('targetAmount').isFloat({ min: 10000 })
      .withMessage('Minimum group target is ₦10,000'),
    body('contributionAmount').isFloat({ min: SAVINGS_CONFIG.group.minAmount })
...
router.post('/group/invite',
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const adminCheck = await dbQuery(
      'SELECT role FROM tenant.group_savings_members WHERE group_id = $1 AND user_id = $2',
      [groupId, userId]
    );
 ...
 */
router.get('/group/members/:groupId',
  authenticateToken,
  validateTenantAccess,
  asyncHandler(async (req: any, res: any) => {
    ...
    const memberCheck = await dbQuery(
      'SELECT * FROM tenant.group_savings_members WHERE group_id = $1 AND user_id = $2',
      [groupId, userId]
    );
...
router.post('/auto/enable',
  ...
  [
    ...
    body('savingsAccountId').optional(),
  ],
  asyncHandler(async (req: any, res: any) => {
...
router.put('/auto/settings',
  ...
  validateTenantAccess,
  [
    body('percentage').optional().isFloat({
      min: SAVINGS_CONFIG.sayt.minPercentage,
      max: SAVINGS_CONFIG.sayt.maxPercentage
    }),
