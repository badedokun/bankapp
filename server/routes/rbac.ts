...
import { loadRBACPermissions, requirePermission, requireAdmin, RBACRequest } from '../middleware/rbac';

export function createRBACRouter(pool: Pool): express.Router {
  const router = express.Router();
  const rbacService = new RBACService(pool);

  // Apply middleware to all routes
  router.use(authenticateToken);
  router.use(loadRBACPermissions(pool));

  /**
   * GET /api/rbac/permissions - Get current user's permissions
   */
  router.get('/permissions', async (req: RBACRequest, res) => {
    try {
   ...
   */
  router.get('/available-features', async (req: RBACRequest, res) => {
    try {
      if (!req.user || !req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.get('/role-based-metrics', async (req: RBACRequest, res) => {
    try {
      if (!req.user || !req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.post('/check-permission', async (req: RBACRequest, res) => {
    try {
      if (!req.user || !req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.get('/roles', requirePermission('role_management', 'read'), async (req: RBACRequest, res) => {
    try {
      if (!req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.post('/assign-role', requirePermission('role_management', 'write'), async (req: RBACRequest, res) => {
    try {
      if (!req.user || !req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.delete('/remove-role', requirePermission('role_management', 'write'), async (req: RBACRequest, res) => {
    try {
      if (!req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.get('/audit-trail', requirePermission('audit_trail_access', 'read'), async (req: RBACRequest, res) => {
    try {
      if (!req.tenant) {
        return res.status(401).json({
          success: false,
      ...
      if (resource) {
        queryParams.push(resource as string);
        query += ` AND a.resource = $${++paramCount}`;
      }

      if (action) {
        queryParams.push(action as string);
   ...
   */
  router.get('/admin/role-permissions/:roleCode', requirePermission('role_management', 'read'), async (req: RBACRequest, res) => {
    try {
   ...
   */
  router.put('/admin/role-permissions/:roleCode', requirePermission('role_management', 'write'), async (req: RBACRequest, res) => {
    try {
      const { roleCode } = req.params;
   ...
   */
  router.get('/admin/users', requirePermission('user_management', 'read'), async (req: RBACRequest, res) => {
    try {
      if (!req.tenant) {
        return res.status(401).json({
          success: false,
   ...
   */
  router.post('/admin/create-role', requirePermission('role_management', 'write'), async (req: RBACRequest, res) => {
    try {
      ...
      try {
        ...
        if (permissions.length > 0) {
          for (const perm of permissions) {
            ...
            if (permResult.rows.length > 0) {
              await client.query(
                `INSERT INTO tenant.rbac_role_permissions (tenant_id, role_id, permission_id, permission_level)
                 VALUES ($1, $2, $3, $4)`,
                [req.tenant.id, roleId, permResult.rows[0].id, perm.level || 'read']
              );
            }
   ...
   */
  router.get('/admin/dashboard', requirePermission('role_management', 'read'), async (req: RBACRequest, res) => {
    try {
      if (!req.tenant) {
        return res.status(401).json({
          success: false,
