...
export function getMockResponse(intent: string): MockResponse {
  return mockResponses[intent] || mockResponses.default;
}

export function getAllMockIntents(): string[] {
  return Object.keys(mockResponses).filter(key => key !== 'default');
}
