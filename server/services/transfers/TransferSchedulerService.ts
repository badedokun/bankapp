...
export class TransferSchedulerService {
    ...
    public static getInstance(): TransferSchedulerService {
     ...
     */
    private async executeScheduledTransfer(scheduledTransfer: any): Promise<void> {
        ...
        try {
            ...
            await client.query(
                `UPDATE tenant.scheduled_transfers
                ...
                await client.query(
                    `UPDATE tenant.scheduled_transfers
                     SET status = 'failed',
                         ...
                         updated_at = CURRENT_TIMESTAMP
                     WHERE id = $1`,
                    [scheduledTransfer.id, result.message || 'Transfer failed']
                );
                ...
                await client.query(
                    `UPDATE tenant.scheduled_transfers
                     SET status = 'failed',
                         ...
                         updated_at = CURRENT_TIMESTAMP
                     WHERE id = $1`,
                    [scheduledTransfer.id, error.message]
                );
            } catch (updateError) {
     ...
     */
    private async processRecurringTransfers(): Promise<void> {
        try {
            const now = new Date();
     ...
     */
    private async executeRecurringTransfer(recurringTransfer: any): Promise<void> {
        const client = await pool.connect();
        ...
        try {
            ...
            if (result.success) {
                ...
                const nextDate = this.calculateNextExecutionDate(
                    new Date(recurringTransfer.next_execution_date),
...
export default TransferSchedulerService.getInstance();
