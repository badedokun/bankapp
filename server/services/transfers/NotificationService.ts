...
interface NotificationEvent {
  ...
  tenantId: string;
  userId: string;
  type: 'transfer_initiated' | 'transfer_completed' | 'transfer_failed' | 'transfer_reversed' | 'receipt_generated';
...
interface NotificationChannel {
  type: 'push' | 'sms' | 'email' | 'in_app' | 'webhook';
  enabled: boolean;
  config?: any;
}
...
interface NotificationPreference {
  userId: string;
  tenantId: string;
  transferInitiated: NotificationChannel[];
  transferCompleted: NotificationChannel[];
  transferFailed: NotificationChannel[];
  receiptGenerated: NotificationChannel[];
  quietHours: {
    enabled: boolean;
...
class NotificationService extends EventEmitter {
   ...
   */
  private async generateNotificationBody(notification: NotificationEvent): Promise<string> {
   ...
   */
  private async generateSMSMessage(notification: NotificationEvent): Promise<string> {
    const body = await this.generateNotificationBody(notification);
    return `FMFB: ${body}`;
  }
   ...
   * Generate email HTML
   */
  private async generateEmailHTML(notification: NotificationEvent): Promise<string> {
    const { type, data } = notification;
    const body = await this.generateNotificationBody(notification);
...
<body>
  ...
  private async getDailyNotificationCount(userId: string, tenantId: string): Promise<number> {
    try {
      const result = await this.db.query(`
  ...
  private async getUserDeviceTokens(userId: string, tenantId: string): Promise<any[]> {
    try {
      const result = await this.db.query(`
   ...
   */
  async getUnreadNotifications(userId: string, tenantId: string): Promise<any[]> {
    try {
      const result = await this.db.query(`
