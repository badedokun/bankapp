...
interface SWIFTTransferRequest {
  ...
  valueDate: string;
  exchangeRate?: number;
  charges: 'OUR' | 'BEN' | 'SHA'; // OUR=sender pays, BEN=beneficiary pays, SHA=shared
}

interface SWIFTTransferResponse {
...
class InternationalTransferService {
  ...
  private complianceApiUrl: string;
   ...
   */
  async processTransfer(request: InternationalTransferRequest, userId: string): Promise<TransferResponse> {
    ...
    try {
      ...
      if (!complianceResult.isCompliant) {
        throw new ValidationError(
          `Transfer blocked by compliance: ${complianceResult.reason}`,
          'compliance'
        );
      }
  ...
  private async performComplianceChecks(
    ...
    try {
      ...
      if (suspiciousPurposes.some(purpose =>
        request.purpose.toLowerCase().includes(purpose)
      )) {
   ...
   */
  private async getExchangeRate(fromCurrency: string, toCountry: string): Promise<CurrencyExchangeRate> {
    try {
   ...
   */
  private async calculateInternationalFees(
    amount: number,
    ...
    const correspondentFeeRates: Record<string, number> = {
   ...
   */
  private async sendSWIFTMessage(request: SWIFTTransferRequest): Promise<SWIFTTransferResponse> {
    try {
      // Mock SWIFT API call for demo
      // In production, integrate with actual SWIFT API

      const mockResponse: SWIFTTransferResponse = {
  ...
  private async reverseFailedTransfer(
    ...
    const transferResult = await client.query(
      'SELECT total_amount FROM international_transfers WHERE id = $1',
      [transferId]
    );
  ...
  private async validateTransferRequest(
    ...
    if (!request.recipientIban?.trim()) {
      throw new ValidationError('Recipient IBAN is required', 'recipientIban');
    }

    if (!request.recipientSwiftCode?.trim()) {
      throw new ValidationError('Recipient SWIFT code is required', 'recipientSwiftCode');
    }

    if (!request.recipientCountry?.trim()) {
      throw new ValidationError('Recipient country is required', 'recipientCountry');
    }

    if (!request.recipientCity?.trim()) {
      throw new ValidationError('Recipient city is required', 'recipientCity');
    }

    if (!request.recipientAddress?.trim()) {
      throw new ValidationError('Recipient address is required', 'recipientAddress');
    }

    if (!request.purpose?.trim()) {
      throw new ValidationError('Transfer purpose is required', 'purpose');
    }

    if (!request.sourceOfFunds?.trim()) {
      throw new ValidationError('Source of funds is required', 'sourceOfFunds');
    }
    ...
    if (request.amount < 1000) {
      throw new ValidationError('Minimum international transfer amount is ₦1,000', 'amount');
    }

    if (request.amount > 10000000) {
      throw new ValidationError('Maximum international transfer amount is ₦10,000,000', 'amount');
    }
  ...
  private async checkInternationalLimits(
    accountId: string,
    amount: number,
    client: any
  ): Promise<void> {
