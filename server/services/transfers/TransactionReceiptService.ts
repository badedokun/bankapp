...
class TransactionReceiptService {
  ...
  private async getTransactionData(
    ...
    let query = '';
    let params: any[] = [transactionId, tenantId];
   ...
   */
  private async generateReceiptNumber(tenantId: string, client: any): Promise<string> {
   ...
   */
  private async generateReceiptFile(receipt: TransactionReceipt, tenantId: string): Promise<void> {
    try {
      ...
      const filePath = path.join(this.receiptStoragePath, tenantId, fileName);
   ...
   */
  private generateReceiptHTML(receipt: TransactionReceipt): string {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  async searchTransactionRecords(
    ...
    try {
      ...
      const records: TransactionRecord[] = dataResult.rows.map(row => ({
        id: row.id,
   ...
   * Get receipt by ID
   */
  async getReceipt(receiptId: string, tenantId: string): Promise<TransactionReceipt | null> {
    try {
  ...
  private async ensureStorageDirectory(): Promise<void> {
    try {
      await fs.mkdir(this.receiptStoragePath, { recursive: true });
    } catch (error) {
