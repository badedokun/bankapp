...
interface ACHConfig {
  ...
  achProcessorUrl: string; // Stripe or Dwolla endpoint
  achProcessorApiKey: string;
}

interface PlaidAccountVerificationResponse {
  accounts: Array<{
    account_id: string;
    name: string;
    mask: string;
...
export class ACHProvider extends BasePaymentProvider {
  ...
  private achProcessorClient: AxiosInstance | null = null;
  private achConfig: ACHConfig | null = null;
  ...
  async initialize(config: Record<string, any>): Promise<void> {
    ...
    this.achProcessorClient = axios.create({
   ...
   */
  private async mockPlaidVerification(request: AccountValidationRequest): Promise<{ accountName: string }> {
   ...
   */
  private async getBankNameFromRouting(routingNumber: string): Promise<string> {
